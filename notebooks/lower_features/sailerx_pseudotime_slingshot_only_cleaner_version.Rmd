...
```{r}
SlingshotDataSet(sds) %>% slotNames
```

```{r tags=c()}
SlingshotDataSet(sds) %>% slot("curves")
```

```{r}
getCurves(SlingshotDataSet(sds))
```

```{r}
getCurves(SlingshotDataSet(sds))@metadata$curves$Lineage1$s %>%
as.data.frame %>%
plot
  # ggplot() +
  #   geom_point(
...
SlingshotDataSet %>% 
getCurves
)@metadata$curves$Lineage2$s %>% 
plot
```

```{r}
df <- getCurves(SlingshotDataSet(sds))@metadata$curves$Lineage1$s %>%
  as.data.frame()
...
```{r}
df2 <- df %>% as_tibble %>% add_row(.before = 1)
df <- df %>% as_tibble %>% add_row()
names(df2) <- c("x", "y")
df2 %>% head
```
...
data_lines %>% dim
data_lines %>% drop_na %>% dim
```

```{r}
FeaturePlot(sobj, features = "F13a1", coord.fixed = T) + geom_segment(data = data_lines, aes(x = x, y = y, xend = xend, yend = yend))
```
...
```{r}
FeaturePlot(sobj, features = defining_genes, coord.fixed = T)
```

```{r}
FeaturePlot(sobj, features = defining_genes, coord.fixed = T, order = T)
```

```{r}
FeaturePlot(sobj, features = c("Mecom", "Ngp"), coord.fixed = T, blend = T, combine = F, pt.size = 2, order = T)[[3]]
```
...
```{r}
traj_lineages <- list()
for (trajectory in names(traj_dict)) {
  if (trajectory == "4") {
    struct_to_use <- sds
  } else {
    struct_to_use <- sce[[trajectory]]
  }
  lineage_nbr <- traj_dict[[trajectory]][["lvl2"]]
    
  df <- SlingshotDataSet(struct_to_use)@curves[[str_c("Lineage", lineage_nbr)]]$s %>% as.data.frame()
  # df <- getCurves(SlingshotDataSet(struct_to_use))@metadata$curves[[str_c("Lineage", lineage_nbr)]]$s %>% as.data.frame()
...
```{r tags=c()}
getCurves(SlingshotDataSet(struct_to_use))@metadata$curves#[[
  #   str_c("Lineage", lineage_nbr)
  # ]]$s
```
...
```{r}
df <- getCurves(SlingshotDataSet(struct_to_use))@metadata$curves[[str_c("Lineage", "1")]]$s %>% as.data.frame()
names(df) <- c("xend", "yend")
df <- df %>% as_tibble()
df %>% head
```
...
```{r}
df <- getCurves(SlingshotDataSet(struct_to_use))@metadata$curves[[str_c("Lineage", "2")]]$s %>% as.data.frame()
names(df) <- c("xend", "yend")
df <- df %>% as_tibble()
df %>% head
```
...
```{r}
getCurves(SlingshotDataSet(struct_to_use))
```
...
```{r}
getCurves(SlingshotDataSet(struct_to_use))@metadata
```
