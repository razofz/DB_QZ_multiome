...
cell_whitelist_path = f"{output_dir}cell_whitelist.tsv"
diverse_metadata_path = f"{output_dir}diverse_metadata.tsv"
rna_pca_diverse_path = f"{output_dir}rna_pca_diverse.tsv"
atac_features_path = f"{output_dir}atac_features.tsv"
atac_matrix_path = f"{output_dir}atac_matrix.mm"
inputs = {
    "their_h5": f"{os.getenv('PROJECT_PATH')}/data/adhoc/SAILERX/SAILERX/data/pbmc10k.hdf5"
}

outputs = {
    "h5_file": f"{output_dir}diverse.h5"
}
...
#
# Using 10X's method, slightly altered (CSR instead of CSC).
...
# %%
filtered_matrix_h5 = f"{os.getenv('PROJECT_PATH')}/data/raw/aggregate/2021_083_Qinyu_agg/outs/filtered_feature_bc_matrix.h5"
...
# %% [markdown]
# ## Import barcodes for Diverse cells kept in former analysis:

# %%
# !wc -l {cell_whitelist_path}

# %%
# diverse_cells =
with open(cell_whitelist_path, "r") as f:
    diverse_cells = [x.strip() for x in f.readlines()]
...
# %%
diverse_cells[:5], len(diverse_cells)

# %%
sum(el in barcodes_h5 for el in diverse_cells)

# %%
len(set(barcodes_h5).difference(set(diverse_cells)))

# %%
sorted(list(set(barcodes_h5).intersection(set(diverse_cells)))) == diverse_cells

# %% [markdown]
# ## Do subset of cells to only cells included in former analysis

# %%
save_cells = np.where(np.in1d(barcodes_h5, diverse_cells))[0]
...
# %% [markdown]
# ## Import metadata for Diverse cells

# %%
diverse_metadata = pd.read_table(diverse_metadata_path, index_col="barcode")

# %%
diverse_metadata.head()

# %% [markdown]
# ## Import PCA embeddings for RNA data

# %%
pca = pd.read_table(rna_pca_diverse_path, index_col="barcode")
pca

# %%
assert all(pca.index == diverse_cells)
...
# %%
atac = mmread(atac_matrix_path)

# %%
atac

# %%
with open(atac_features_path, "r") as f:
...
standard_chromosome_intervals = np.where(
    bool_std_chrs
    # np.char.startswith(np.array(features_h5), "chr")
)[0]
...
own_h5.create_dataset_like(
    "cells/labels", their_h5["cells/labels"], shape=len(diverse_cells)
)

# %%
name = "cells/rna_depth"
own_h5.create_dataset_like(name, their_h5[name], shape=len(diverse_cells))
...
# %%
own_h5["cells/labels"][:] = list(diverse_metadata["seurat_clusters"])
...
# %%
own_h5["cells/rna_depth"][:] = list(diverse_metadata["nCount_RNA"])
