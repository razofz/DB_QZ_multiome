...
# Subset to only diverse cells

```{r}
diverse_cells <- (sobj[[]] %>% rownames)[sobj[[]] %>% rownames %>% str_sub(start=-1) %>% str_detect("1") %>% which]
diverse_cells %>% length
```

```{r}
sobj <- sobj %>% subset(cells = diverse_cells)
```
...
```{r}
thresholds[[feat]][["high"]] <- 16e3
thresholds[[feat]][["low"]] <- 1.2e3
```
...
```{r}
thresholds[[feat]][["high"]] <- 2
thresholds[[feat]][["low"]] <- .3
```
